...
def gen_lhs_samples(knobs, nsamples):
    ...
    for knob in knobs:
        ...
        if knob['vartype'] == VarType.ENUM:
            # 离散值的边界需要扩展0.5，保证边界概率和中间取值的概率均等
            maxvals.append(float(knob['maxval']) + 0.5)
            minvals.append(float(knob['minval']) - 0.5)
        else:
    ...
    nfeats = len(knobs)
    LOG.info('xyq nfeats = %d !!!', nfeats)
    LOG.info('xyq samples = %d !!!', nsamples)
    LOG.info('xyq criterion = %s !!!', 'maximin')
    samples = lhs(nfeats, samples=nsamples, criterion='maximin')
    maxvals = np.array(maxvals)
    minvals = np.array(minvals)
    scales = maxvals - minvals
    # LOG.info('xyq samples: %s ', '%s' % samples)
    # LOG.info('xyq maxvals: %s ', '%s' % maxvals)
    # LOG.info('xyq minvals: %s ', '%s' % minvals)
    # LOG.info('xyq types: %s ', '%s' % types)
    for fidx in range(nfeats):
        LOG.info('xyq fidx = #%d !!!!!!!!!', fidx)
        # 仅对整数或实数操作
        # 取对数要求大于0、只对最大最小值之商大于1000倍的knob范围取对数
        # 最小值为0时，最大值大于1000也取对数，计算对数时最小值用1计算
        ...
        LOG.info('xyq type %d in %s ? %s', types[fidx],
                 '%s' % list((VarType.INTEGER, VarType.REAL)),
                 '%s' % check_type)
        LOG.info('xyq minval %s > 0 ? %s',
                 '%s' % minvals[fidx],
                 '%s' % (check_minval or check_minval_zero))
        LOG.info('xyq maxval %s/minval %s=%s > 100 ? %s',
                 '%s' % maxvals[fidx],
                 '%s' % minvals[fidx],
                 '%s' % (maxvals[fidx]/minvals[fidx]),
                 '%s' % check_quotient)
        if check_type and (check_minval or check_minval_zero) and check_quotient:
            ...
            # LOG.info('xyq min and max: %s %s', '%s' % minvals[fidx], '%s' % maxvals[fidx])
            # LOG.info('xyq logmin and logmax: %s %s', '%s' % logmin, '%s' % logmax)
            LOG.info('xyq samples #%d (log): %s', fidx, '%s' % samples[:, fidx])
        else:
            samples[:, fidx] = uniform(loc=minvals[fidx], scale=scales[fidx]).ppf(samples[:, fidx])
            LOG.info('xyq samples #%d: %s', fidx, '%s' % samples[:, fidx])
    lhs_samples = []
